...
**A. GitHub Actions（`.github/workflows/deploy-infra.yml`）**

`infra/**` に変更が入った main への push で自動実行される（`workflow_dispatch` で手動実行も可）。
事前にリポジトリの Secrets への登録が必要:

| Secret | 内容 |
|---|---|
| `CLOUDFLARE_API_TOKEN` | Workers Scripts / R2 Storage / Secrets Store の Write + ゾーンの DNS Write |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare アカウント ID |
| `SVELTIA_GITHUB_CLIENT_ID` | OAuth App の Client ID（Secrets 名に `GITHUB_` プレフィックスは使えないため `SVELTIA_` 付き） |
| `SVELTIA_GITHUB_CLIENT_SECRET` | OAuth App の Client Secret |

**B. ローカル CLI**（初回は Cloudflare へのインタラクティブログインあり）:
...
ステートは Cloudflare 側（`Cloudflare.state()`）に保存されるため、CI とローカルどちらから実行しても状態は共有される。
...
- **MDX**: `extension: md` のため、`.mdx` 記事はこの管理画面には表示されない。
