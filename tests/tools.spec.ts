...
test.describe("Tools Page", () => {
	...
	test("should lead with Nix and the dotfiles repository", async ({ page }) => {
		...
		await expect(
			page.getByRole("heading", { name: "なぜ Nix か" }),
		).toBeVisible();

		const dotfiles = page.locator(
			'a[href="https://github.com/ta93abe/dotfiles"]',
		);
	...
	test("should omit encyclopedic and discontinued tools", async ({ page }) => {
		...
		await expect(
			page.getByRole("heading", { name: "Spark AR Studio" }),
		).toHaveCount(0);
		await expect(page.getByRole("heading", { name: "Yaak" })).toHaveCount(0);
		await expect(
			page.getByRole("heading", { name: "Google Chrome" }),
		).toHaveCount(0);
