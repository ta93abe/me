...
describe("email and slack payloads", () => {
	...
	it("escapes HTML in the inbox body", () => {
		const email = buildInboxEmail({
			fromAddress: "contact@ta93abe.com",
			fromName: "Takumi Abe",
			inboxEmail: "inbox@example.com",
			...
			email: validFields.email,
			subject: "a < b",
			message: "hello <em>there</em>",
		});
		expect(email.html).not.toContain("<script>");
		expect(email.html).toContain("&lt;script&gt;");
		expect(email.html).toContain("a &lt; b");
		expect(email.html).toContain("hello &lt;em&gt;there&lt;/em&gt;");
	});
...
describe("handleContactRequest", () => {
	...
	it("returns 503 without creating a workflow when Turnstile is not configured", async () => {
		const create = vi.fn();
		const response = await handleContactRequest(jsonRequest(validFields), {
			TURNSTILE_SECRET: "",
			CONTACT_WORKFLOW: { create },
