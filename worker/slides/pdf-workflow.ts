	...
	async run(
		...
		await step.do(
			...
			async () => {
				...
				await this.env.CONTENT.put(stagingKey, await response.arrayBuffer(), {
		...
		await step.do("put-r2", async () => {
			const object = await this.env.CONTENT.get(stagingKey);
			if (!object) {
				throw new Error(`staging pdf missing: ${stagingKey}`);
			}
			await this.env.CONTENT.put(key, object.body, {
				httpMetadata: { contentType: "application/pdf" },
				customMetadata: { version, slug },
			});
			await this.env.CONTENT.delete(stagingKey);
			return { key };
		});
